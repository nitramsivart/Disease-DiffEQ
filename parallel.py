...
def dowork(x, u):
  ...
  for xprime in xpoints:
    ...
    total = total + coefficient * (B(x*xresolution-xprime*xresolution) * u[xprime])
  return total * xresolution / 3
...
def integral(u):
  ...
  arr = [u] * len(xpoints)
  return array(pool.map(dowork, xpoints, arr))
